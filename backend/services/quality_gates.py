...
def compute_vision_overlap(
    ...
    if not vision_labels:
        return 0.0, ""
    xml_set = {l.lower().strip() for l in xml_labels if l and l.strip()}
    vision_set = {l.lower().strip() for l in vision_labels if l and l.strip()}
    if not xml_set:
        return 0.0, ""
    overlap = len(xml_set & vision_set) / len(xml_set)
    extra = [l for l in vision_labels if l and l.lower().strip() not in xml_set]
...
# Gate 4: Voice input relevance guard (used by session webhook)
_ARCH_KEYWORDS = {
